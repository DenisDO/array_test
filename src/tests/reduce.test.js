...
describe('tests for method reduce', () => {
  ...
  test('Method shouldn\'t mutate initial array', () => {
    ...
    const secondArr = new MyArray(1, 2, 3);
    const mockCallback = jest.fn();
    arr.reduce(mockCallback);

    expect(arr).toEqual(secondArr);
  });
  ...
  test(`If array is empty and InitialValue is specified, callback shouldn't 
        be called, and methods result should be ItitialValue`, () => {
    const arr = new MyArray();
  ...
  test.only(`the number of callback function calls should be equal to the 
             arr.length with init value and arr.length-1 without`, () => {
